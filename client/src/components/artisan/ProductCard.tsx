...
export default function ProductCard({ title, price, stock, image, category, index }: ProductProps) {
  return (
    <motion.div
    ...
    >
      <div className="relative h-64 w-full overflow-hidden">
        ...
        <div className="absolute bottom-4 right-4 flex gap-2 translate-y-10 group-hover:translate-y-0 transition-transform duration-300">
          <Button size="icon" className="h-10 w-10 rounded-full bg-white/20 backdrop-blur-md border border-white/30 hover:bg-[#D4AF37] hover:border-[#D4AF37] text-white">
            <Edit className="w-4 h-4" />
          </Button>
          <Button size="icon" className="h-10 w-10 rounded-full bg-white/20 backdrop-blur-md border border-white/30 hover:bg-red-500 hover:border-red-500 text-white">
